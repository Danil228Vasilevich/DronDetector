...
class Detect:
    def __init__(self, list_band: list[RangeFreq]) -> None:
        self._list_band: list[RangeFreq] = list_band
    ...
    def start(self) -> None:
        start_task = PowerTask(self._list_band[0], 100)
        self._power_sweep = SweepPower(self._data_call_back, start_task)
        self._event_detecting_thread = Thread.Event()
        self._detecting_thread = Thread.Thread(
